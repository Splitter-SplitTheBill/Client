...
export default function EditProfileScreen(props) {
  ...
  function addAcc() {
    ...
    axios({
      method: "PATCH",
      url: `http://192.168.1.5:3000/users/${userData._id}/accounts`,
      headers: {
  ...
  function deleteAcc(accId) {
    ...
    axios({
      method: "PATCH",
      url: `http://192.168.1.5:3000/users/${userData._id}/accounts/${accId}`,
      headers: {
        token: userData.token
      }
    })
      ...
      <SafeAreaView style={styles.container}>
        <ScrollView>
          <View style={{height: '100%', width: '100%'}}>
            ...
            >
              <View style={{height: 150, width: 150, borderRadius: 999, justifyContent: 'center', alignItems: 'center', backgroundColor: 'green'}}>
                <Image style={{height: '95%', width: '95%', borderRadius: 999, overflow: 'hidden'}} source={{uri: image_url}} />
              </View>
              <TouchableOpacity
            ...
            <View style={{width: '100%', alignItems: 'center'}}>
              ...
              {accounts.map(acc => {
                return (
                  <View
                    style={{ flexDirection: "row", justifyContent: "center", width: '92%'}}
                  >
                    <Text style={styles.AccDetailTypeInstance}>{acc.instance}</Text>
                    <Text style={styles.AccDetailType}>{acc.name}</Text>
                    <Text style={styles.AccDetailType}>
                      {acc.accountNumber}
                    </Text>
                    <TouchableOpacity style={{width: '7%', height: 50, justifyContent: 'center', alignItems: 'center'}} onPress={() => deleteAcc(acc._id)}>
                      <Image
                ...
                <View style={{ marginTop: 10 }}>
                  ...
                  <View>
                    ...
                    <TextInput
                      ...
                      style={styles.inputLogin}
                    ></TextInput>
                    <TouchableOpacity onPress={() => addAcc()} style={{justifyContent: 'center', alignItems: 'center'}}>
